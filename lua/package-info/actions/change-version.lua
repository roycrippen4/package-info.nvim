...
---@return string?
local function get_change_version_command(dependency_name, version)
...
---@param dependency_name string - dependency for which to get the command
---@return string?
local function get_version_list_command(dependency_name)
  if config.options.package_manager == constants.PACKAGE_MANAGERS.pnpm then
...
---@return nil
local function display_dependency_version_select(version_list, dependency_name)
  dependency_version_select.new({
    ...
    on_submit = function(selected_version)
      ...
      job({
        command = get_change_version_command(dependency_name, selected_version),
        on_start = function()
          loading.start(id)
        end,
        on_success = function()
...
function M.run()
  ...
  job({
    json = true,
    command = get_version_list_command(dependency_name),
    on_start = function()
      loading.start(id)
    end,
